...
export function UnitWiseReportColorTable<TData, TValue>({
  columns,
  data,
  searchKey,
  tableLabel,
  ...
  const exportPdfHandler = async () => {
    const doc = new jsPDF();
    const header: string[] = columns.map((val: any) => val.header as string);
    // .splice(1);
    ...
    const body = pdfData.map((row) => {
      return header.map((col) => {
        const smallLetterCols = col.toLowerCase().replaceAll(/ /g, "_");
  ...
  className={(() => {
    ...
    const rate = record?.rate_of_accidents_in_1_lakhs_km ?? 0;
    const isTotal = record?.depot_name === "Total";

    if (isTotal) return "bg-gray-300 font-bold";
    if (rate === 0) return "bg-green-100";
    if (rate <= 0.25) return "bg-lime-100";
    if (rate <= 0.5) return "bg-yellow-100";
    if (rate <= 0.75) return "bg-orange-100";
    return "bg-red-100";
  })()}
>
