"use client";
import { Input } from "@/components/ui/input";
import { Autocomplete, TextField } from "@mui/material";
import React, { useState } from "react";

const dummyData = [
  {
    accedent_ref_no: "1563867",
    accedent_date: "06/05/2025",
    bus_no: "FA3465",
  },
];

const AccedentInspectorForm = () => {
  const [selectedAccedentData, setSelectedAccedentData] = useState<{
    accedent_ref_no: string;
    accedent_date: string;
    bus_no: string;
  } | null>();

  return (
    <div className="flex flex-col gap-3 p-5 mb-10">
      <h6 className="text-2xl">Inspector Report</h6>
      <div className="grid grid-cols-4 gap-10 items-end pb-5 mt-8">
        {/* 1st row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">Accident Reference Number</label>
          <Autocomplete
            size="small"
            options={dummyData}
            getOptionLabel={(option) => option.accedent_ref_no}
            onChange={(_, val) => setSelectedAccedentData(val)}
            renderInput={(params) => (
              <TextField
                {...params}
                variant="outlined"
                className="text-[12px]"
              />
            )}
          />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Date Of Accident</label>
          <Input
            placeholder="Date"
            value={
              selectedAccedentData ? selectedAccedentData.accedent_date : ""
            }
            onChange={() => console.log("clicked")}
          />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Bus No</label>
          <Input
            value={selectedAccedentData ? selectedAccedentData.bus_no : ""}
            onChange={() => console.log("clicked")}
          />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Inspector Name</label>
          <Input />
        </div>
        {/* 2nd row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">FIR Registered (Yes/ No)</label>
          <div className="flex items-center justify-evenly">
            <label htmlFor="#isFir">Yes</label>
            <Input type="radio" name="isFir" className="w-6" />
            <label htmlFor="#isFir">No</label>
            <Input type="radio" name="isFir" className="w-6" />
          </div>
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">FIR Number</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Under Section Details</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Accused in the Accident In FIR</label>
          <Input />
        </div>
        {/* 3rd row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">FIR Case Against KSRTC (Yes/No)</label>
          <div className="flex items-center justify-evenly">
            <label htmlFor="#isfiragainstksrtc">Yes</label>
            <Input type="radio" name="isfiragainstksrtc" className="w-6" />
            <label htmlFor="#isfiragainstksrtc">No</label>
            <Input type="radio" name="isfiragainstksrtc" className="w-6" />
          </div>
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            Date Of Bus Released From Police Station
          </label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Enquiry Police person Name</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Enquiry Police Phone Number</label>
          <Input />
        </div>
        {/* 4th row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">Digital Evidence </label>
          <Input type="file" />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Witness</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            Primary Cause Of Accident In Insp Report
          </label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            Responsibility Of The Accident In Insp Report
          </label>
          <Input />
        </div>
        {/* 5th row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            If Not KSRTC Is Responsible, Action Taken
          </label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            Whether FIR IS MODIFIED OR NOT. Responisbilty Changed From KSRTC As
            Per Evidence Submitted
          </label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            Total No. Of Days Bus Docked Due To Accident
          </label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Revenue Loss Due To Dock Of Bus</label>
          <Input />
        </div>
        {/* 6th row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">Inspector Report Additional Details</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            Whether The Bus Have VALID Insurance Or Not{" "}
          </label>
          <Input placeholder="Date" />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Type Of Insurance</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Insurance Claim Applied (Yes/No)</label>
          <div className="flex items-center justify-evenly">
            <label htmlFor="#isinsuranceclaimapplied">Yes</label>
            <Input
              type="radio"
              name="isinsuranceclaimapplied"
              className="w-6"
            />
            <label htmlFor="#isinsuranceclaimapplied">No</label>
            <Input
              type="radio"
              name="isinsuranceclaimapplied"
              className="w-6"
            />
          </div>
        </div>
        {/* 7th row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">Insurance Company Name</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Policy Number</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Claim Number </label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Date Of Insurance Claim Applied</label>
          <Input />
        </div>
        {/* 8th row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm">Name Of Spot Surveyor</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Spot Surveyor Phone Number</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">
            Final Bill Submitted Insurance Co. (Y/N)
          </label>
          <div className="flex items-center justify-evenly">
            <label htmlFor="#is_insurance_submit_final_bill">Yes</label>
            <Input
              type="radio"
              name="is_insurance_submit_final_bill"
              className="w-6"
            />
            <label htmlFor="#is_insurance_submit_final_bill">No</label>
            <Input
              type="radio"
              name="is_insurance_submit_final_bill"
              className="w-6"
            />
          </div>
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Final Bill Amount by KSRTC (Rs.)</label>
          <Input />
        </div>
        {/* 9th row */}
        <div className="flex flex-col gap-2">
          <label className="text-sm"> Approved Amount By Insurance Co.</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Admitted Amount By Insurance Co.</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Payment Settled (Y/N)</label>
          <Input />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm">Date of Payment Settled</label>
          <Input />
        </div>
        {/* 10th row */}
        <div className="flex flex-col gap-2 col-span-2">
          <label className="text-sm">REMARKS</label>
          <Input />
        </div>
      </div>
      <div className="flex gap-3 ms-auto me-30">
        <button className="px-4 py-1 rounded-md bg-red-700 text-white">
          Cancel
        </button>
        <button className="px-4 py-1 rounded-md bg-green-700 text-white">
          Submit
        </button>
      </div>
    </div>
  );
};

export default AccedentInspectorForm;
